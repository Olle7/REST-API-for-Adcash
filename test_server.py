...
class Test_server(unittest.TestCase):
    ...
    def test_invalid_request_to_all_categories_page(self):
    ...
    def test_add_new_category(self):#adds new category, which name is "C1"
    ...
    def test_all_categories_page(self):#checks whether category with name "C1" is in database.
        url = "http://127.0.0.1:5000/json"
        ...
        for catergory in response_decoded_json.json():
            if catergory["name"]=="C1":
                return
        raise Exception
    def test_delete_category(self):#deletes category "C1"
        url = "http://127.0.0.1:5000/json"
        response_decoded_json = requests.get(url, data={},headers={"Content-type": "application/x-www-form-urlencoded"})
        assert (response_decoded_json.status_code == 200)
        ...
        assert(delete_message=="category deleted")
