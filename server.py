    ...
    if product_id == "new" and "name" in request.values and len(request.values) == 1:  # adds new product.
        ...
        for category_index_in_database in range(len(categories)):
            if categories[category_index_in_database]["id"] == category_id:
                ...
                categories[category_index_in_database]["products"].append(
                    {"name": request.values.get("name"), "id": new_id})
                json.dump(categories, f, indent=4)
                f.truncate()  # remove remaining part
                return app.response_class(response=json.dumps({"id": new_id}), mimetype='application/json', status=201)
        abort(404)
    elif "name" in request.values and len(request.values)==1 or len(request.values)==0:  # edit or delete product.
        try:
            category_id=int(category_id)
            ...
            if categories[category_index_in_database]["id"] == category_id:
                for product_index_in_database in range(len(categories[category_index_in_database]["products"])):
                    if categories[category_index_in_database]["products"][product_index_in_database]["id"] == product_id:
                        if len(request.values)==0:#return product
                            print("väljastada toode")
                            return app.response_class(response=json.dumps({"id":categories[category_index_in_database]["products"][product_index_in_database]["id"],"name":categories[category_index_in_database]["products"][product_index_in_database]["name"]}), mimetype='application/json', status=201)
                        elif request.values.get("name"):  # edit product name
